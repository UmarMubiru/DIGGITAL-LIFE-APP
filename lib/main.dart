...
import 'package:google_mobile_ads/google_mobile_ads.dart'; //importation for google ads package
import 'package:flutter/foundation.dart' show kIsWeb;

Future<void> main() async {
  WidgetsFlutterBinding.ensureInitialized();
  await Firebase.initializeApp(options: DefaultFirebaseOptions.currentPlatform);
  // Initialize the Google Mobile Ads SDK on supported platforms only
  if (!kIsWeb) {
    ...
    return MultiProvider(
      ...
      child: Consumer<UserProvider>(
        builder: (context, user, _) => MaterialApp(
                ...
                if (isHW) {
                  ...
                  WidgetsBinding.instance.addPostFrameCallback((_) {
                    if (context.mounted) {
                      Navigator.of(
                        context,
                      ).pushNamedAndRemoveUntil('/hw', (route) => false);
                    }
                  });
          ...
          // Only showing the fixed onGenerateRoute section:
          onGenerateRoute: (settings) {
            ...
            if (settings.name == '/hw/chat/detail') {
              ...
              if (roomId == null || roomId.isEmpty) {
                return MaterialPageRoute(
                  builder: (_) => const _RouteErrorScreen(
                    message: 'Missing or invalid chat room ID',
                  ),
                  settings: settings,
                );
              }
              return MaterialPageRoute(
                builder: (_) => HWChatDetailScreen(roomId: roomId),
                settings: settings,
              );
            }
            return null;
...
// add near the end of file (below RootApp) — simple routing error screen
